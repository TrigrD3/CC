<?php 

include "config.php";
include "head.php";

?>

<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
      <!-- Bootstrap CSS -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous" />
    <!-- My CSS -->
    <link rel="stylesheet" href="assets/Style.css" />
    <!-- reader Setting CSS -->
    <link rel="stylesheet" href="assets/stylesettingreader.css" />
    <title>Setting HonBox</title>

  </head>
  <body> 
    <section class='umum'>
        <div class='umum'>
            <ul id="menu">
                <li>User Setting</li>
            </ul>  
        </div>
        <div>
            <ul id="menu">
                <li><a href="halamansettingaccountsetting.php">Account</a></li>
                <li><a href="bookmarksetting.php">Bookmark</a></li>
                <li><a href="historysetting.php">History</a></li>
                <li><a href="readersetting.php">Reader</a></li>
            </ul>  
            <hr id=i></hr>
        </div>
    </section>
    <div class='umum'>
        <table>
            <tr>
            <td>Reader Settings</td>
            </tr>
        </table>
    </div>

    <!-- Site Theme -->
    <section class='umum'>
        <div id="detail">Site Theme</div>
        <div id="detail">
            <label class="container">Default
                <input type="checkbox" checked="checked">
                <span class="checkmark"></span>
              </label>
              <label class="container">Dark
                <input type="checkbox">
                <span class="checkmark"></span>
              </label>
              <label class="container">Light
                <input type="checkbox">
                <span class="checkmark"></span>
              </label>
        </div>
        <hr></hr>
    </section>

    <!-- Reading Style -->
    <section class='umum'>
        <div id="detail">Reading Style</div>
        <div id="detail">
            <label class="container">Default
                <input type="checkbox" checked="checked">
                <span class="checkmark"></span>
              </label>
              <label class="container">Page
                <input type="checkbox">
                <span class="checkmark"></span>
              </label>
              <label class="container">List
                <input type="checkbox">
                <span class="checkmark"></span>
              </label>
        </div>
        <hr></hr>
    </section>

    <!-- Images Per Page -->
    <section class='umum'>
        <div id="detail">Images Per Page</div>
        <div id="detail">
            <label class="container">Default
                <input type="checkbox" checked="checked">
                <span class="checkmark"></span>
            </label>
            <label class="container">???????
                <input type="checkbox">
                <span class="checkmark"></span>
            </label>
            <label class="container">Load 3 Images Per Page
                <input type="checkbox">
                <span class="checkmark"></span>
            </label>
            <label class="container">Load 6 Images Per Page
                <input type="checkbox">
                <span class="checkmark"></span>
            </label>
            <label class="container">Load 10 Images Per Page
                <input type="checkbox">
                <span class="checkmark"></span>
            </label>
        </div>
    </section>
    
  </body>
</html>